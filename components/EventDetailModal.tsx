...
import {
  ...
  TouchableOpacity,
  View,
} from "react-native"; // Importa Share e Platform
...
const EventDetailModal: React.FC<EventDetailModalProps> = ({
  ...
  const handleLinkPress = () => {
    if (event.external_link) {
      ...
      const url =
        event.external_link.startsWith("http://") ||
        event.external_link.startsWith("https://")
          ? event.external_link
...
END:VCALENDAR`;
      ...
      alert(
        "Il file del calendario è stato scaricato. Aprilo per aggiungere l'evento.",
      );
      return;
    }
  ...
  const shareEvent = async () => {
    ...
    const shareMessage =
      `Ciao, ho trovato un evento interessante: ${event.title}!\n\n` +
    ...
    try {
      ...
      // Ho corretto qui
      // Aggiungi un controllo di tipo per usare la proprietà 'message' in modo sicuro
        ...
        <View style={styles.modalContainer}>
          <ScrollView>
            ...
            <Text style={styles.details}>
              {formattedDate} - {event.parrocchie.name}
            </Text>
